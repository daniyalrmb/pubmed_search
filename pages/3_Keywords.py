...
# Using beautiful soup to scrape abstract, keywords, title, etc for all 100 pubmed articles
  ...
  df['abstract_without_stopwords'] = df['Abstract'].apply(lambda x: ' '.join([word for word in x.split() if word.lower() not in (stop_words)]))

  
  def test(df):
    df['counts']=Counter(df["abstract_without_stopwords"].lower().split()).most_common(10)
    return df
